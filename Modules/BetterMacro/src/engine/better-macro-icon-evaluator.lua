-- Dynamic macro icons do not need a full catalogue pass every frame. The old
-- rate was elapsedMs * 2 (roughly 2,000 macro evaluations per second), which
-- repeatedly parsed the same 150 records. A small time budget keeps changes
-- responsive while capping both normal work and post-hitch bursts.
local MacrosToUpdatePerSecond = 240
local MaxMacrosPerFrame = 12
local UpdateBudget = 0
...
function MegaMacroIconEvaluator.Update(elapsedMs)
    UpdateBudget = UpdateBudget + ((tonumber(elapsedMs) or 0) / 1000) * MacrosToUpdatePerSecond
    local macrosToScan = math.min(MaxMacrosPerFrame, math.floor(UpdateBudget))
    if macrosToScan < 1 then return end
    UpdateBudget = UpdateBudget - macrosToScan
