...
local function TryImportGlobalMacros()
    ...
    if newNumberOfGlobalMacros > numberOfGlobalMacros then
        print("BetterMacro: Global import created " .. newNumberOfGlobalMacros - numberOfGlobalMacros .. " macros.")
    end
...
local function TryImportCharacterMacros()
    ...
    if newNumberOfCharacterMacros > numberOfCharacterMacros then
        print("BetterMacro: Character import created " .. newNumberOfCharacterMacros - numberOfCharacterMacros .. " macros.")
    end
...
local function MergeCharacterSpecializationMacros()
    ...
    local specializationData = MegaMacroCharacterData.Specializations[MegaMacroCachedSpecialization]
    local characterSpecializationMacros = specializationData and specializationData.Macros
...
function MegaMacroEngine.FindAvailableGlobalMacro()
    if not InCombatLockdown() then
        ...
        print("BetterMacro: Failed to find available global macro slot.")
        return nil
    end
end
...
function MegaMacroEngine.FindAvailableCharacterMacro()
    if not InCombatLockdown() then
        ...
        if hasFreeSlot then
            ...
            -- print("BetterMacro: Created new character macro at index " .. index .. ".")
            return index
        end
...
function MegaMacroEngine.SafeInitialize()
    ...
    -- This event runs after every loading screen. Install the global
    -- PickupMacro wrapper once instead of nesting another copy each time.
    if Initialized then
        return true
    end
...
function MegaMacroEngine.VerifyMacros()
    ...
    for i=1, numberOfGlobalMacros do
        ...
        if macroId then
            if macroId > MacroLimits.MaxGlobalMacros and macroId < MacroIndexOffsets.Inactive then
                print("BetterMacro: Found character macro in global space! " .. i .. " " .. name .. " #" .. macroId)
                DeleteMacro(i)
            end
        end
    end
    ...
    for i=1 + MacroIndexOffsets.NativeCharacterMacros, numberOfCharacterMacros + MacroIndexOffsets.NativeCharacterMacros do
        ...
        if macroId then
            if macroId < MacroIndexOffsets.NativeCharacterMacros then
                print("BetterMacro: Found global macro in character space! " .. i .. " " .. name .. " #" .. macroId)
                DeleteMacro(i)
            end
        end
    end
    ...
    local function VerifyMacro(macro, i)
        ...
        if macroIds[macroId] then
            -- print("BetterMacro: Found duplicate macroId " .. macroId .. " " .. macro.DisplayName)
        else
            macroIds[macroId] = true
        end
        -- Check for missing macroIndex
        if not macroIndex then
            print("BetterMacro: Fixing missing macro " .. macro.Id .. " " .. macro.DisplayName)
            BindNewMacro(macro)
        end
...
function MegaMacroEngine.Uninstall()
    ...
    InitializeMacroIndexCache()
    message("BetterMacro: Uninstalled. Disabled BetterMacro and reload your UI.")
    return true
end
