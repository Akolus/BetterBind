...
local BINDPAD_SLOT_INCREMENT = 40
local BINDPAD_MAXSLOTS_DEFAULT = BINDPAD_SLOT_INCREMENT
-- Only used when importing the historical flat character-slot tables.
local BINDPAD_LEGACY_MIGRATION_SLOTS = isRetail and 49 or 42
local BINDPAD_LAYOUT_COLUMNS = 8
local BINDPAD_LAYOUT_ICON = 50
local BINDPAD_LAYOUT_GAP = 11
...
function BindPadFrame_OnLoad(self)
    PanelTemplates_SetNumTabs(BindPadFrame, 5)

    SlashCmdList["BINDPAD"] = BindPad_SlashCmd
    SLASH_BINDPAD1 = "/betterbind"
    SLASH_BINDPAD2 = "/bb"
...
function BindPadFrame_OutputText(text)
    ChatFrame1:AddMessage("[BetterBind] " .. text, 1.0, 1.0, 0.0)
end
...
function BindPadSlot_OnEnter(self)
    ...
    if not BindPadCore.CursorHasIcon() then
        GameTooltip:AddLine(BINDPAD_TOOLTIP_CLICK_USAGE, 0.8, 1.0, 0.8)
    end
...
function BindPadMacroAddButton_OnClick(self)
    -- Retained as a compatibility stub for the legacy XML template. The
    -- control is hidden and BetterBind accepts drops through the slot itself.
    return
end
...
function BindPadFrame_ShowHotkeyToggle(self)
    ...
    for i = 1, BindPadCore.useBindPadSlot or 0 do
        local button = _G["BindPadSlot" .. i]
        if button then
            BindPadSlot_UpdateState(button)
        end
...
function BindPadCore.CreateBindPadSlot(usenum)
    for i = min(usenum + 1, BindPadCore.useBindPadSlot + 1), max(usenum, BindPadCore.useBindPadSlot) do
        ...
        if i <= usenum then
            local col = mod(i - 1, BINDPAD_LAYOUT_COLUMNS)
            local row = floor((i - 1) / BINDPAD_LAYOUT_COLUMNS)
            button:ClearAllPoints()
            button:SetPoint(
                "TOPLEFT",
                BindPadSlotButtonContainer,
                "TOPLEFT",
                15 + col * (BINDPAD_LAYOUT_ICON + BINDPAD_LAYOUT_GAP),
                -17 - row * (BINDPAD_LAYOUT_ICON + BINDPAD_LAYOUT_GAP)
            )
            button:SetSize(BINDPAD_LAYOUT_ICON, BINDPAD_LAYOUT_ICON)
            button:Enable()
            button:Show()
        else
            button:Hide()
    ...
    local rows = max(1, ceil(usenum / BINDPAD_LAYOUT_COLUMNS))
    local gridWidth = BINDPAD_LAYOUT_COLUMNS * BINDPAD_LAYOUT_ICON
        + (BINDPAD_LAYOUT_COLUMNS - 1) * BINDPAD_LAYOUT_GAP
    local gridHeight = rows * BINDPAD_LAYOUT_ICON + (rows - 1) * BINDPAD_LAYOUT_GAP
    BindPadSlotButtonContainer:SetSize(gridWidth + 30, gridHeight + 20)

    BindPadScrollFrameFooter:ClearAllPoints()
    BindPadScrollFrameFooter:SetPoint("TOPLEFT", BindPadSlotButtonContainer, "TOPLEFT", 0, -gridHeight - 20)
...
function BindPadShowLessSlotButton_OnClick()
    local tabInfo = BindPadCore.GetTabInfo(BindPadVars.tab)
    tabInfo.numSlot = max(BINDPAD_MAXSLOTS_DEFAULT, tabInfo.numSlot - BINDPAD_SLOT_INCREMENT)
    BindPadFrame_OnShow()
end

function BindPadShowMoreSlotButton_OnClick()
    local tabInfo = BindPadCore.GetTabInfo(BindPadVars.tab)
    tabInfo.numSlot = tabInfo.numSlot + BINDPAD_SLOT_INCREMENT
    BindPadFrame_OnShow()
end

local function NormalizeSlotCount(tabInfo)
    local current = tonumber(tabInfo.numSlot) or BINDPAD_MAXSLOTS_DEFAULT
    if current >= BINDPAD_MAXSLOTS_DEFAULT and mod(current, BINDPAD_SLOT_INCREMENT) == 0 then
        return tabInfo
    end

    local highestUsed = 0
    for id, slot in pairs(tabInfo) do
        if type(id) == "number" and type(slot) == "table"
            and (slot.type or slot.action or slot.name or slot.texture)
        then
            highestUsed = max(highestUsed, id)
        end
    end

    local nearest = floor((current + BINDPAD_SLOT_INCREMENT / 2) / BINDPAD_SLOT_INCREMENT)
        * BINDPAD_SLOT_INCREMENT
    local required = ceil(highestUsed / BINDPAD_SLOT_INCREMENT) * BINDPAD_SLOT_INCREMENT
    tabInfo.numSlot = max(BINDPAD_MAXSLOTS_DEFAULT, nearest, required)
    return tabInfo
end
...
function BindPadCore.GetTabInfo(tab)
    if tab == BINDPAD_GENERAL_TAB then
        ...
        return NormalizeSlotCount(BindPadVars)
    else
        local character = BindPadCore.character
        local profileNum = BindPadCore.GetCurrentProfileNum()
        ...
        if not profile[tabname] then
            profile[tabname] = {}
            -- Only tabs 2-4 existed in the legacy flat character table.
            -- Tab 5 is new and must always start with its own empty storage.
            if tab <= 4 then
                for newid = 1, BINDPAD_LEGACY_MIGRATION_SLOTS do
                    local oldid = newid + (tab - 2) * BINDPAD_LEGACY_MIGRATION_SLOTS
        ...
        return NormalizeSlotCount(profile[tabname])
    end
end

function BindPadCore.GetSlotInfoInTab(tab, id, newFlag)
    if not BindPadCore.character then
        BindPadFrame_OutputText("DEBUG: Something wrong. Please report this message to the author of BetterBind.")
        return nil
    end
